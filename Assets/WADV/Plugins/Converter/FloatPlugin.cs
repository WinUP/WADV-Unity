...
namespace WADV.Plugins.Converter {
    /// <inheritdoc />
    /// <summary>
    /// <para>用于将目标值解析为32位浮点数的插件</para>
    /// <list type="bullet">
    ///     <listheader><description>必选匿名参数</description></listheader>
    ///     <item><description>要转换的数据</description></item>
    /// </list>
    /// </summary>
    /// <remarks>插件仅会解析第一个参数，如果没有参数传入则返回32位浮点数0。第一个参数若包含无法被转换的结构必定出现错误。</remarks>
    [StaticRegistrationInfo("Float")]
    [UsedImplicitly]
    public class FloatPlugin : IVisualNovelPlugin {
        public Task<SerializableValue> Execute(PluginExecuteContext context) {
            ...
            return Task.FromResult<SerializableValue>(new FloatValue {value = 0});
        }
